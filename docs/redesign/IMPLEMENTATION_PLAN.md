...
| 9 | Reimbursements | ☐ | |
| 10 | Settings hub + 6 sub-pages **(large)** | ☑ | 10a–10i landed 2026-05-04; iPhone walkthrough on 10b looked good |
...
>
> **10b (`290ecfd`)** — hub built against v8 measurements (frame "Settings · Hub", y≈9450–10300). Layout: `IdentityHero` (44×44 initials avatar in lime, name, mono email) → sync-status card (`bg-surface` + `border-rule`, lime dot + `CONNECTED · BANK · SYNC X MIN AGO`, 3-col stat grid ACCOUNTS/TXNS/RULES wired to live data) → three rooms (MANAGE: ⎌ Accounts & Sync · ◱ Categorization · ⚙ Preferences ; DATA: ↓ Export ; DANGER: ◷ Account · △ Reset data with `text-warn` glyph) using new `SettingsRoomRow` primitive (24px mono glyph slot · 14.5px label · 12.5px meta · `›` arrow). Version footer reads a hardcoded `v0.1.0` (Phase 11 to wire build-time injection); MEMBER SINCE pulls `firebaseUser.metadata.creationTime`. Sticky LOG OUT footer (`text-warn`) at `bottom-[68px]` on mobile (above TabBar) and `bottom-0 left-[60px]` on desktop (clear of SideRail) — placement matches resolved Q4. DATA folds ICS Import into the single Export row per resolved route shape. iPhone-verified.
>
> **10c (`109c604`)** — `/settings/accounts` wraps the existing BankConnectionCard + IcsImportCard (kept as-is per the strategy: rewrite shadcn shells flat in a later pass). v8 puts ICS in this room as `CREDIT CARD IMPORT`, so this checkpoint shifts ICS out of Export entirely. Adds a SYNC SETTINGS room with three local-state Toggle rows (Auto-sync on app open, Background refresh, Sync notifications) and a PENDING JOBS placeholder. New primitives: `Toggle` (36×20 pill matching v8 accent track + 14px circle) and `ToggleRow` (label/meta + trailing Toggle). Sync toggles persist via local `useState`; 10e wires them to `useUserPreferences` in a follow-up.
>
> **10d (`cd516dd`)** — `/settings/categorization` to v8's IA: 3-col `StatGrid` (RULES/MERCHANTS/UNCATEG.), an ACTIONS room (3 buttons firing `useRecategorizeTransactions` with toast results: rules-only / AI uncategorized / AI all), a BROWSE room (Rules → `#rules` anchor · Merchants → `#merchants` anchor · Categories → `/categories`), and a HOW IT WORKS info box. The Rules and Merchants anchors render the existing `CategorizationRulesCard` + `BuiltInRulesCard` inline (card chrome retained — flat rewrite later). `SettingsRoomRow` was extended with `onClick` + `badge` + `disabled` props so action rows reuse the same chrome as navigation rows.
>
> **10e (`f42a858`)** — new `useUserPreferences` / `useUpdateUserPreferences` hooks reading/writing a `preferences` field on `users/{uid}` (piggy-backs on the existing user-doc rule rather than introducing a `meta/` rule — out of scope per brief). Optimistic mutations roll back on permission errors. `/settings/preferences` matches v8 sections DISPLAY · PRIVACY · NUMBERS · NOTIFICATIONS. Theme renders read-only "DARK" per resolved Q2 (light is being deleted in Phase 11); Currency read-only "EUR · €". New `ValueRow` primitive (label/meta + inline shadcn Select trigger styled as `value · ›`) handles Default tab / Month starts on / Budget alerts. ToggleRow handles Blur amounts / App lock / Round to whole € / Hide tiny amounts. Most prefs persist but are not yet consumed elsewhere — Phase 11 wires `fiscalMonthStart` into Home days-left and `blurAmounts` into `formatAmount`.
>
> **10f (`e8d93d6`)** — `/settings/export` to v8: AVAILABLE banner (txn count + min/max date span computed inline), RANGE filter rows (read-only stubs showing "All time / All / All banks" with toast on tap; real filters out of scope), FORMAT cards for CSV / JSON with leading chips and inline `↓ EXPORT` triggers wired to the existing `exportTransactionsAsCSV/JSON` helpers, and a disabled SHARE row ("Email to accountant — coming soon"). ICS deliberately not here.
>
> **10g (`8307ca1`)** — `/settings/account` is minimal per resolved Q4: identity hero + SIGN-IN section (email + provider — `google.com` / `password` / etc) + HISTORY section (member-since + last-sign-in from `firebaseUser.metadata`). LOG OUT intentionally absent (lives only on the hub footer); a small footnote points users at it.
>
> **10h (`f3f1277`)** — `/settings/danger` with the new `TypedConfirmDialog` primitive gating each destructive action behind verbatim phrase entry per resolved Q3. Two wired actions: Reset categories (new inline `useResetCategories` mutation — clears `categoryId` on every txn in 500-doc writeBatch chunks; phrase `RESET CATEGORIES`) and Reset transaction data (wraps `useResetTransactionData`; phrase `RESET DATA`). Two stub rows disabled with "coming soon" meta: Delete all data (needs a transactional Cloud Function) and Delete account (needs Auth admin call).
>
> **10i (`<this>`)** — delete folded-in legacy components and the old flat settings page: `src/pages/Settings.tsx`, `src/components/settings/AutoCategorizationCard.tsx`, `src/components/settings/DataExportCard.tsx`, `src/components/settings/DangerZoneCard.tsx`. Survivors stay alive because the new screens still embed them: `BankConnectionCard` + `IcsImportCard` (10c), `CategorizationRulesCard` + `BuiltInRulesCard` (10d). Their flat-rewrite is Phase 11 cleanup. `npm run typecheck`, `npm run lint`, `npm run build` all clean. All 7 routes verified rendering against the seeded test user.
