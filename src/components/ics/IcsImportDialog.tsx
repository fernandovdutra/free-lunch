...
interface IcsImportDialogProps {
  ...
  /** Called after a statement is successfully imported. */
  onImported?: () => void;
}

/**
 * Controlled dialog for the full ICS PDF import flow
 * (select → parse → preview → import → success/error). Extracted so both the
 * ICS overview page and individual missing-month rows can trigger the same
 * flow without duplicating the state machine.
 */
